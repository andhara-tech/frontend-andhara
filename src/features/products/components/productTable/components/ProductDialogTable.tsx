...
import { productsSchema, type ProductFormValues } from "@/features/products/schema/productsShema"
...
export function ProductDialog() {
  ...
  const form = useForm<ProductFormValues>({
    ...
    defaultValues: {
      ...
      stock: BRANCHES.map((branch) => ({ id_branch: branch.id_branch, quantity: 0 })),
    },
  })
  ...
  useEffect(() => {
    if (selectedProduct) {
      ...
      const stockValues = BRANCHES.map((branch) => {
        const stockItem = selectedProduct.stock.find((item) => item.id_branch === branch.id_branch)
        return {
      ...
      form.reset({
        ...
        id_supplier: selectedProduct.id_supplier ?? (supplierOptions.length > 0 ? supplierOptions[0].id : ""),
        stock: stockValues,
      })
    } else {
      form.reset({
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} disabled value={selectedProduct.id_product} />
                    </FormControl>
                  </FormItem>
                )}
              />
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          {field.value ? "El producto está activo" : "El producto está inactivo"}
                        </FormDescription>
                      </div>
                      <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormMessage />
                        <input type="hidden" {...form.register(`stock.${index}.id_branch`)} value={branch.id_branch} />
                      </FormItem>
                    )}
                  />
                ))}
